...
chats = {}
day = 86400
bot_idrami = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idrami[1])
...
local rami_add_sudo = redis:get('RAMBO:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if rami_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
...
local rami_add_sudo = redis:get('RAMBO:'..bot_id..'sudoo'..user_id..'')
if rami_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('RAMBO:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('RAMBO:'..bot_id..'admins:', user_id)  
local owner = database:sismember('RAMBO:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('RAMBO:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('RAMBO:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('RAMBO:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('RAMBO:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('RAMBO:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('RAMBO:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
k2342 = io.open("RAMBO.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'RAMBO:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
time = database:get("RAMBO:time:ads"..bot_id..chat_id)
time2 = database:get("RAMBO:up:ads"..bot_id)
if (time2 and time2 ~= os.date("%x%I")) then
h = http.request("http://api-victor.ml/rami/ads.php?get=RAMBO")
if h and h:match("(.*)RAMBO(.*)") then 
h = JSON.decode(h)
h = h.text
database:set("RAMBO:text:ads"..bot_id..chat_id,h)
if not RAMBO_stop then 
text = text .. "\n"..h
RAMBO_stop = "ok"
end
database:set("RAMBO:up:ads"..bot_id,os.date("%x%I"))
end
...
h = JSON.decode(h)
h = h.text
database:set("RAMBO:text:ads"..bot_id,h)
if not RAMBO_stop then 
text = text .. "\n"..h
RAMBO_stop = "ok"
end
...
database:set("RAMBO:time:ads"..bot_id..chat_id,os.date("%x%H"))
if not database:get("RAMBO:gr:not:ads:"..bot_id..chat_id..os.date("%x")) then 
if not RAMBO_stop then 
text = text .. "\n"..(database:get("RAMBO:text:ads"..bot_id) or "")
RAMBO_stop = "ok"
end
else
x = math.random(1, 2)
...
local rami = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
rami = rami .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق رامبو لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
end
if disable_web_page_preview then
rami = rami .. '&disable_web_page_preview=true'
end
if text then
rami = rami..'&text='..URL33.escape(text2)
end
if parse_mode then
rami = rami .. '&parse_mode='..parse_mode
end
rami = rami..'&reply_markup='..JSON.encode(keyboard)
https.request(rami)
else
tdcli_function ({
ID = "SendMessage",
...
if value == "lock" then
function rami333(extra,result,success)
...
getUser(msg.sender_user_id_, rami333)
end
if value == "prore" then
...
local rami2 = database:get("add"..bot_id)
if rami2 then
local rami = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(rami)
local data = json:decode(stats)
if (data.result and data.result.status == 'creator') then
if not database:get( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_) then
...
local rami222 = ''..msg.sender_user_id_..''
local ress = https.request(rami222)
if ress then
...
local rami = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(rami)
local data2 = json:decode(stats)
local ramin = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(ramin) or tonumber(data2.result) > tonumber(ramin))) then
if database:get( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_) then
function RAMBO_info(k1,k2)
...
if not database:get( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_,true)
function RAMBO_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."💢┇  تم تفعيل البوت في مجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,RAMBO_info)
database:sadd("RAMBO:gog"..bot_id, msg.chat_id_)
function RAMBO_info2(k1,k2)
function dl_cb222(t1,t2)
database:set('RAMBO:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("RAMBO"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل البوت \n📟┇ايدي المنشئ ⌁≻ ("..msg.sender_user_id_..")\n☑️┇يوزر المنشئ ⌁≻ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n📟┇ايدي المجموعه ⌁≻ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ⌁≻ ("..k2.title_..")\n📎┇رابط المجموعه ⌁≻ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('RAMBO:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'RAMBO:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('RAMBO:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
local tsX_o = database:get("lock_tag:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇ التاك مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـتاك ❌")
database:set("lock_tag:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الشارحه مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـشارحه ❌")
database:set("lock_sarha:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل تعديل الميديا") then
local tsX_o = database:get("RAMBO:lo:edit:new:"..bot_id..msg.chat_id_)
if tsX_o then
tsX000("lock",msg,"✔┇تعديل الميديا مقفول ❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 تعديل الـميديا ❌")
database:set("RAMBO:lo:edit:new:"..bot_id..msg.chat_id_,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الكلايش مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـكلايش ❌")
database:set("lock_word:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") then
local tsX_o = database:get("lock_edit:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇التعديل مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـتعديل ❌")
database:set("lock_edit:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇التكرار مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـتكرار ❌")
database:set("lock_lllll:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇المتحركه مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـمتحركه ❌")
database:set("lock_gif:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الملفات مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـملفات ❌")
database:set("lock_files:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الماركدون مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـماركدون ❌")
database:set("lock_mark:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الصور مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـصور ❌")
database:set("lock_photo:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الملصقات مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـملصقات ❌")
database:set("lock_stecker:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الفيديو مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـفيديو ❌")
database:set("lock_video:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الانلاين مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الانلاين ❌")
database:set("lock_inline:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الدردشه مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـدردشه ❌")
database:set("lock_chat:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇التوجيه مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـتوجيه ❌")
database:set("lock_fwd:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇التثبيت مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـتثبيت ❌")
database:set("lock_pin:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الاغاني مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الاغاني ❌")
database:set("lock_audeo:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الصوت مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـصوت ❌")
database:set("lock_voice:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الجهات مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـجهات ❌")
database:set("lock_contact:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇بالفعل تم قفل العربيه❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـعربيه ❌")
database:set("lock_ar:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇بالفعل تم قفل الانكليزيه❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الانكليزيه ❌")
database:set("lock_en:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الميديا مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـميديا ❌")
database:set("lock_media:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔️┇الروابط مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـروابط ❌")
database:set("lock_link:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل اليوزر") then
local tsX_o = database:get("lock_username:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇اليوزر مقفول❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـيوزر ❌")
database:set("lock_username:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الاشعارات مقفوله❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الاشعارات ❌")
database:set("lock_new:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇البوتات بالطرد مقفوله❗️ ")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـبوتات بالطرد ❌")
database:set("lock_botAndBan:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇بالفعل تم قفل البوتات❗️")
else
tsX000("lock",msg,"⚙ ¦¬ تـم قـفل 🔐 الـبوتات ❌")
database:set("lock_bot:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:RAMBO"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_new:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_tag:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:RAMBO"..msg.chat_id_..bot_id)
end
end
...
database:del("RAMBO:lo:edit:new:"..bot_id..msg.chat_id_)
end
end
...
database:del("lock_sarha:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_lllll:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_gif:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_files:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_mark:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_photo:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_stecker:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_video:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_inline:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fwd:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_audeo:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_voice:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_botAndBan:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_contact:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:RAMBO"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_username:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_link:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:RAMBO"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End RAMBO unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:RAMBO"..msg.chat_id_..bot_id) then 
local RAMBO_wr = (database:get("RAMBO:not:word:"..bot_id..msg.chat_id_) or 100)
...
if database:get("lock_link:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_bot:RAMBO"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
return "stop"
end
if database:get("lock_botAndBan:RAMBO"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("lock_fwd:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('RAMBO:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:RAMBO"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:RAMBO"..msg.chat_id_..bot_id) then
local hash = 'flood:RAMBO:'..bot_id..msg.chat_id_
if not database:get(hash) then
floodRAMBO = 10
else
floodRAMBO = tonumber(database:get(hash))
end
local hash = 'RAMBO:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
local chat = msg.chat_id_
user_id = msg.sender_user_id_
o = database:get("RAMBO:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
...
database:set("lock_link:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:RAMBO"..msg.chat_id_..bot_id,"ok")
database:set('RAMBO:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:incr("RAMBO:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
database:sadd('RAMBO:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('RAMBO:'..bot_id..'leave:groups') then
...
local user_info = database:hgetall('RAMBO:'..bot_id..'user:'..v)
...
if (not database:get('RAMBO:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '💢┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("RAMBO:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "RAMBO"
else
...
local gps = database:scard('RAMBO:'..bot_id.."groups") or 0
local user = database:scard('RAMBO:'..bot_id.."userss") or 0
local gps2 = database:scard("RAMBO:gog"..bot_id) or 0
local gps3 = database:scard('RAMBO:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('RAMBO:'..bot_id.."groups") or 0
local gps2 = database:smembers("RAMBO:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("RAMBO:gog"..bot_id, gps[i])
database:set('RAMBO:'..bot_id.."enable:"..gps[i],true)
database:set( 'RAMBO:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("RAMBO:gog"..bot_id) or 0
for i=1,#gps do
database:del("RAMBO:gog"..bot_id)
database:del('RAMBO:'..bot_id.."enable:"..gps[i])
database:del( 'RAMBO:'..bot_id.."charge:"..gps[i])
end
...
database:del("RAMBO:gog"..bot_id)
database:del('RAMBO:'..bot_id.."groups")
end
end 
if text:match("^روابط الكروبات$") then
local gpss = database:smembers("RAMBO:"..bot_id.."groups") or 0
local gps = database:scard('RAMBO:'..bot_id.."groups")
text = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('RAMBO:'..bot_id.."group:link"..gpss[i])
text = text.."|"..i.."| ⌁≻ "..gpss[i].."\n ⌁≻ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('RAMBO.txt', 'w')
f:write(text)
f:close()
local RAMBOe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. RAMBOe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'RAMBO.txt' .. '"'
io.popen(curl)
end
if text:match("^تحديث$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '- اهلآ عزيزي المـطور تم تحديٰث البوت ♻️', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.github.com/ramizob3a/RAMBO/master/libs/utils.lua')
os.execute('rm -rf RAMBO.lua')
os.execute('wget https://raw.github.com/ramizob3a/RAMBO/master/RAMBO.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'RAMBO:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'RAMBO:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:del("RAMBO:gog"..bot_id,txt[2])
...
local rami_info = nil
function rami333(extra,result,success)
rami_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan1 = 2592000
database:setex( 'RAMBO:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:setex( 'RAMBO:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function rami333(extra,result,success)
rami_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'RAMBO:'..bot_id.."charge:"..txt[2],true)
...
local files_RAMBO = database:smembers("files"..bot_id)
local rami = io.popen('cd files_RAMBO && ls'):read("*all")
local files_RAMBO2 = ''
...
local ramij = {string.match(text, "^(تعين الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,ramij[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..ramij[2], 1, 'html')
end
end
end
...
local kt = RAMBOee.rami_RAMBO(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('RAMBO:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('RAMBO:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'RAMBO:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'RAMBO:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'RAMBO:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'RAMBO:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('RAMBO:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'RAMBO:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'RAMBO:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('RAMBO:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '💢┇قم بارسال المنشور من القناة', 1, 'md')
database:del('RAMBO:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
database:set("RAMBO:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,RAMBO_edis[1])
elseif (database:get("RAMBO:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id)) then 
send(msg.chat_id_, msg.id_, 1, "☑┇ تم الحفظ بنجاح", 1, 'md')
database:set("RAMBO:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("RAMBO:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("RAMBO:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("RAMBO:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
database:del("RAMBO:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id)
end  
if (text and text == 'مسح كليشه المبرمج') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('RAMBO:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح الكليشه ', 1, 'html')
return "RAMBO"
end
if text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
database:del("RAMBO:edit:text:su:new2:"..bot_id..msg.chat_id_..database:get("RAMBO:edit:text:su:new:"..bot_id..msg.chat_id_..t[1]))
database:del("RAMBO:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
local hash =  'RAMBO:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('RAMBO:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
database:sadd('RAMBO:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
function promote_by_reply(extra, result, success)
local hash =  'RAMBO:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
database:sadd('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("RAMBO:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'RAMBO')
return "RAMBO"
end
if text and is_owner(msg) and database:get("RAMBO:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("RAMBO:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,  '💢┇ تم الحفض بنجاح', 1, 'md')
database:set("RAMBO:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("RAMBO:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if not is_creator(msg) and database:get("RAMBO:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "RAMBO"
end
function ban_by_reply(extra, result, success)
local hash =  'RAMBO:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('RAMBO:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:sadd('RAMBO:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
database:set('RAMBO:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
if result.id_ then
database:srem('RAMBO:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('RAMBO:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'RAMBO:'..bot_id..'gbanned:'
...
local hash =  'RAMBO:'..bot_id..'gmuted:'
if is_admin(result) then
...
local hash =  'RAMBO:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
database:sadd('RAMBO:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:sadd('RAMBO:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('RAMBO:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('RAMBO:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
local hash =  'RAMBO:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('RAMBO:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('RAMBO:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("RAMBO:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("RAMBO:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👨┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("RAMBO:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
y = database:get("RAMBO:fel:o:me:"..bot_id..msg.chat_id_) or 10
x = database:get("RAMBO:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_) or 0
send(msg.chat_id_, msg.reply_to_message_id_, 1, "⚠┇تم الابلاغ على الرساله\n⚠┇تبقى {"..(y-(x+1)).."} حتى يتم حذف الرساله", 1, 'html')
database:incr("RAMBO:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)
if (database:get("RAMBO:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)) then 
x = database:get("RAMBO:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)
y = database:get("RAMBO:fel:o:me:"..bot_id..msg.chat_id_) or 10
...
database:set("RAMBO:mute:deleta:msg:"..bot_id..msg.chat_id_,"RAMBOE")
send(msg.chat_id_, msg.id_, 1, "💢┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ" and is_creator(msg)) then 
database:del("RAMBO:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
database:set("RAMBO:fel:o:me:"..bot_id..msg.chat_id_,a[2])
...
local RAMBO_098 = 0
if database:get("RAMBO:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("RAMBO:get:mod:"..bot_id..msg.chat_id_)
...
database:set("RAMBO:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
if not database:get('RAMBO:'..bot_id..'id:mute'..msg.chat_id_) then 
local msgs = database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0
local edit = database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.sender_user_id_
if is_sudo(msg2) then
RAMBO_oop = 'مبرمج البوت'
elseif is_creator(msg) then
RAMBO_oop = 'منشئ الكروب'
elseif (database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("RAMBO:all_if:"..database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
RAMBO_oop = database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if not database:get('RAMBO:'..bot_id..'id:mute'..msg.chat_id_) then 
local msgs = database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0
local edit = database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
...
elseif is_creator(msg) then
RAMBO_oop = 'منشئ الكروب'
elseif (database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("RAMBO:all_if:"..database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
RAMBO_oop = database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_owner(msg2) then
RAMBO_oop = 'اداري الكروب'
...
if text:match("^صورتي (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('RAMBO:'..bot_id..'get:photo'..msg.chat_id_) then
...
database:set('RAMBO:'..bot_id..'flood:RAMBO:'..msg.chat_id_,floodRAMBO[2])
...
database:set("RAMBO:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"),"ok")
...
send(msg.chat_id_, msg.id_, 1, '💥┇ عزيزي المبرمج لايمكنك تعطيل هذا الامر \n📊┇ [يمكنك يوزره المزيد حول الاعلانات هنا](https://t.me/th3victory)', 1, "md") 
end
end
if (text and text == "تفعيل الاعلانات" and is_creator(msg)) then 
database:del("RAMBO:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"))
...
database:set('RAMBO:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ⌁≻  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
if not database:get("RAMBO:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if t2.invite_link_ ~= false then 
send(msg.chat_id_, msg.id_, 1, '📩┇ᏳᖇᎧᏬᎮ ᏝᎨᏁᏥ\n'..(t2.invite_link_ or "Error"), 1, "html")
elseif (database:get('RAMBO:'..bot_id.."group:link"..msg.chat_id_)) then 
send(msg.chat_id_, msg.id_, 1, '📩┇ᏳᖇᎧᏬᎮ ᏝᎨᏁᏥ GROUP\n'..database:get('RAMBO:'..bot_id.."group:link"..msg.chat_id_), 1, "html")
else
...
database:set('RAMBO:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
...
local wel = database:get('RAMBO:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
database:hset('RAMBO:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
...
database:hdel('RAMBO:'..bot_id..'filters:'..msg.chat_id_, rws[2])
...
getChannelMembers(msg.chat_id_,"Kicked",0,10000,ramiididi)
end
...
n = (n + 1)
local user_info = database:hgetall('RAMBO:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if not database:get('RAMBO:'..bot_id..'id:mute'..msg.chat_id_) then
...
database:del('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
else
end
end
if text:match("^جهاتي$") then
add = (tonumber(database:get('RAMBO:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد جهاتك ⌁≻ *{"..add.."}*\n📨┇سيتم مسح العدد بعد هذه الرساله", 1, 'md')
database:del('RAMBO:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^مسح تعديلاتي$") then
edit = (tonumber(database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد تعديلاتك ~⪼ *{"..edit.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد التعديلات)$") or text:match("^(تعديلاتي)$") then
local edit = database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
...
database:set("RAMBO:not:word:"..bot_id..msg.chat_id_,RAMBO_0[2])
end
...
database:del('RAMBO:'..bot_id..'banned:'..msg.chat_id_)
...
database:del('RAMBO:'..bot_id..'creator:'..msg.chat_id_)
...
database:set("RAMBO:lock:ban_and_kick"..bot_id..msg.chat_id_,"RAMBO")
...
database:del("RAMBO:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
if database:get("lock_media:RAMBO"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('RAMBO:'..bot_id..'flood:RAMBO:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('RAMBO:'..bot_id..'flood:RAMBO:'..msg.chat_id_)
end
if not database:get( 'RAMBO:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'RAMBO:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:RAMBO"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:RAMBO"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:RAMBO"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:RAMBO"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
------------
if database:get('RAMBO:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('RAMBO:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('RAMBO:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('RAMBO:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'RAMBO:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = database:ttl( 'RAMBO:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
if not database:get('RAMBO:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '💢┇امر اطردني بالفعل تم تفعيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '💢┇تم تفعيل امر اطردني', 1, 'md')
database:del('RAMBO:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') and is_owner(msg) then
if database:get('RAMBO:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تعطيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
database:set('RAMBO:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end

if text:match("^اطردني$") then
if not database:get('RAMBO:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('RAMBO:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('RAMBO:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
end
end
local yess = redis:get('RAMBO:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('RAMBO:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('RAMBO:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('RAMBO:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('RAMBO:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('RAMBO:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
redis:set('RAMBO:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'texts'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('RAMBO:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
...
local text_sudo = redis:get('RAMBO:'..bot_id..'text_sudo')
local nrami = redis:get('RAMBO:'..bot_id..'nmrami')
local narami = redis:get('RAMBO:'..bot_id..'narami')
if text_sudo then
send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
else
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nrami or 9630), (narami or "الدولة ☜اّْبـُِ✯ُِـنًٍّ الاسد"), "", bot_id)
end
end
for k,v in pairs(sudo_users) do
...
redis:set('RAMBO:'..bot_id..'nrami'..msg.sender_user_id_..'', 'msg')
return false end
end
if text:match("^+(.*)$") then
local ramio = redis:get('RAMBO:'..bot_id..'sudoo'..text..'')
local rami2 = redis:get('RAMBO:'..bot_id..'nrami'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 📝', 1, 'md')
redis:set('RAMBO:'..bot_id..'nmrami', text)
redis:set('RAMBO:'..bot_id..'nrami'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'nrami'..msg.sender_user_id_..'')
if rami2 == 'mmsg' then
send(msg.chat_id_, msg.id_, 1, '• `تم حفظ الاسم يمكنك اظهار الجهة بـ ارسال امر المبرمج` ☑', 1, 'md')
redis:set('RAMBO:'..bot_id..'nrami'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'narami', text)
local nmrami = redis:get('RAMBO:'..bot_id..'nmrami')
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmrami, text , "", bot_id)
return false end
end

if text:match("^رفع مبرمج$")  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
if redis:sismember('RAMBO:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'💢┇بالفعل تم رفعه مبرمج')
else
redis:set('RAMBO:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('RAMBO:'..bot_id..'dev', result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
redis:set('RAMBO:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('RAMBO:'..bot_id..'dev', result.id_)
texts = '💢┇العضو ⌁≻ ['..result.title_..'](t.me/'..(apmd[2] or 'TH3VICTORY')..')\n\n☑┇تم رفعه مبرمج'
else
texts = '✖┇خطاء'
end
...
redis:set('RAMBO:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('RAMBO:'..bot_id..'dev', apmd[2])
...
function demote_by_reply(extra, result, success)
if not redis:sismember('RAMBO:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'💢┇ بالفعل تم تنزيله من المبرمجين')
else
redis:del('RAMBO:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('RAMBO:'..bot_id..'dev', result.sender_user_id_)
...
function demote_by_username(extra, result, success)
if result.id_ then
redis:del('RAMBO:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('RAMBO:'..bot_id..'dev', result.id_)
...
redis:del('RAMBO:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('RAMBO:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'💢┇ تم تنزيله من مبرمجين البوت')
end
if not database:get('RAMBO:'..bot_id..'repowner:mute'..msg.chat_id_) then
local rami = redis:get('RAMBO:'..bot_id..'rami'..text..''..msg.chat_id_..'')
if rami then 
function rami_RAMBO_re(t1,t2)
local user_msgs = database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if is_sudo(msg) then
t = 'مبرمج البوت'
elseif is_creator(msg) then
t = 'منشئ الكروب'
elseif (database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("RAMBO:all_if:"..database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("RAMBO:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
...
local rami = rami:gsub('#username',(t2.username_ or 'لا يوجد')) 
local rami = rami:gsub('#name',(t2.first_name_ or 'لا يوجد'))
local rami = rami:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local rami = rami:gsub('#edit',(edit or 'لا يوجد'))
local rami = rami:gsub('#msgs',(user_msgs or 'لا يوجد'))
local rami = rami:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, rami, 1, 'md')
end
getUser(msg.sender_user_id_, rami_RAMBO_re)
elseif (redis:get('RAMBO:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
elseif (redis:get('RAMBO:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
elseif (redis:get('RAMBO:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':video:'..text..''..msg.chat_id_..''))
elseif (redis:get('RAMBO:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
elseif (redis:get('RAMBO:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end
end
if not database:get('RAMBO:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local rami = redis:get('RAMBO:'..bot_id..'rami'..text..'')
if rami then 
function rami_RAMBO_re(t1,t2)
local user_msgs = database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
...
rami = rami:gsub('#username',(t2.username_ or 'لا يوجد'))
rami = rami:gsub('#name',(t2.first_name_ or 'لا يوجد'))
rami = rami:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
rami = rami:gsub('#edit',(edit or 'لا يوجد'))
rami = rami:gsub('#msgs',(user_msgs or 'لا يوجد'))
rami = rami:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, rami, 1, 'html')
end
getUser(msg.sender_user_id_, rami_RAMBO_re)
elseif (redis:get('RAMBO:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':sticker:'..text))
elseif (redis:get('RAMBO:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':voice:'..text))
elseif (redis:get('RAMBO:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':video:'..text))
elseif (redis:get('RAMBO:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':gif:'..text))
elseif (redis:get('RAMBO:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('RAMBO:'..bot_id..':file:'..text))
end  
end

if text:match("^اضف رد$") and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '💢┇ارسل الكلمه التي تريد اضافتها', 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami1 = redis:get('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'')
if rami1 == 'msg' then
...
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('RAMBO:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('RAMBO:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end

if text:match("^مسح رد$") and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '💢┇ارسل الكلمه التي تريد مسحها', 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local rami1 = redis:get('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'')
if rami1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح الرد', 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('RAMBO:'..bot_id..'rami'..text..''..msg.chat_id_..'')
redis:del('RAMBO:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('RAMBO:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('RAMBO:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('RAMBO:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('RAMBO:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('RAMBO:'..bot_id..'ramire'..msg.chat_id_..'', text)
end
end
...
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'', 'msg')
return false end
local rami1 = redis:get('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'')
if rami1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه ) ', 1, 'md')
send(msg.chat_id_, msg.id_, 1,  '☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد مسجات المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد التعديلات', 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'', 're')
redis:set('RAMBO:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end

if text:match("^مسح رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '💢┇ارسل الكلمه التي تريد مسحها' , 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local rami1 = redis:get('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'')
if rami1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح الرد', 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'', 'no')
redis:del('RAMBO:'..bot_id..':sticker:'..text)
redis:del('RAMBO:'..bot_id..':voice:'..text)
redis:del('RAMBO:'..bot_id..':video:'..text)
redis:del('RAMBO:'..bot_id..':gif:'..text)
redis:del('RAMBO:'..bot_id..':file:'..text)
redis:del('RAMBO:'..bot_id..'rami'..text)
redis:srem('RAMBO:'..bot_id..'ramiresudo', text)
end
end
if text:match("^مسح المبرمجين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('RAMBO:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('RAMBO:'..bot_id..'dev')
redis:del('RAMBO:'..bot_id..'sudoo'..v)
end
send(msg.chat_id_, msg.id_, 1, "💢┇تم مسح مبرمجين البوت", 1, 'md')
end
if text:match("^مسح ردود الاداري$") and is_owner(msg) then
local list = redis:smembers('RAMBO:'..bot_id..'ramire'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('RAMBO:'..bot_id..'ramire'..msg.chat_id_..'', text)
redis:del('RAMBO:'..bot_id..'rami'..v..''..msg.chat_id_)
redis:del('RAMBO:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('RAMBO:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('RAMBO:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('RAMBO:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('RAMBO:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('RAMBO:'..bot_id..'rami'..v..msg.chat_id_)
end
...
local list = redis:smembers('RAMBO:'..bot_id..'ramiresudo')
for k,v in pairs(list) do
redis:del('RAMBO:'..bot_id..'ramiresudo', text)
redis:del('RAMBO:'..bot_id..'rami'..v..'')
redis:del('RAMBO:'..bot_id..':sticker:'..v)
redis:del('RAMBO:'..bot_id..':voice:'..v)
redis:del('RAMBO:'..bot_id..':video:'..v)
redis:del('RAMBO:'..bot_id..':gif:'..v)
redis:del('RAMBO:'..bot_id..':file:'..v)
redis:del('RAMBO:'..bot_id..'rami'..v)
end
...
database:set('RAMBO:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = database:get('RAMBO:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
database:set('RAMBO:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if database:sismember('RAMBO:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('RAMBO:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not database:sismember('RAMBO:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:del('RAMBO:'..bot_id..'leave:groups'..msg.chat_id_)
end
end
if (text and text == 'تعطيل المغادره') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if database:get('RAMBO:'..bot_id..'leave:groups') then
send(msg.chat_id_, msg.id_, 1, '💢┇مغادره بالفعل تم تعطيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '💢┇تم تعطيل مغادره البوت', 1, 'md')
database:set('RAMBO:'..bot_id..'leave:groups'..msg.chat_id_,true)
end
end
if (text and text == "مسح صلاحيه" and is_creator(msg)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("RAMBO:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok")
return "RAMBO"
end  
if (text and database:get("RAMBO:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف صلاحيه بنجاح', 1, 'md')
database:del("RAMBO:all_if:"..text..bot_id..msg.chat_id_)
database:del("RAMBO:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end
if (text and text == "اضف صلاحيه" and is_creator(msg)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("RAMBO:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "RAMBO"
end
if text and database:get("RAMBO:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("RAMBO:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '💢┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `اداري` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("RAMBO:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("RAMBO:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "RAMBO"
end
if (text and database:get("RAMBO:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "اداري" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("RAMBO:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("RAMBO:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("RAMBO:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
database:del("RAMBO:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '💢┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `اداري` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
ts_text = database:get("RAMBO:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('RAMBO:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مميز" then 
database:sadd('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "اداري" then 
database:sadd('RAMBO:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("RAMBO:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function rami33332(extra,result2,success)
info = '💢┇العضو ⌁≻ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'th3victory')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(result.sender_user_id_, rami33332)  
end 
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end 
end
...
ts_text = database:get("RAMBO:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
database:set("RAMBO:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '💢┇العضو ⌁≻ ['..result.title_..'](t.me/'..(apmd[2] or 'TH3VICTORY')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "اداري" then 
database:sadd('RAMBO:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("RAMBO:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:srem('RAMBO:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مميز" then 
database:srem('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "اداري" then 
database:srem('RAMBO:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:del("RAMBO:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('RAMBO:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "مميز" then 
database:srem('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "اداري" then 
database:srem('RAMBO:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '💢┇العضو ⌁≻ ['..result.title_..'](t.me/'..(apmd[2] or 'TH3VICTORY')..')\n☑┇تم تنزيله من '..apmd[1]
database:del("RAMBO:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('RAMBO:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "مميز" then 
database:srem('RAMBO:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "اداري" then 
database:srem('RAMBO:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"☑┇ تم تنزيله من "..apmd[1])
database:del("RAMBO:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('RAMBO:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('RAMBO:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
...
database:del('RAMBO:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
...
database:set('RAMBO:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('RAMBO:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("RAMBO"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("RAMBO"..bot_id..":sudo:gr")
end
if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
local rami_info = nil
function rami333(extra,result,success)
...
local user_msgs = database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local msguser = tonumber(database:get('RAMBO:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 1)
local edit = database:get('RAMBO:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
...
if not database:get('RAMBO:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('RAMBO:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("RAMBO:gr:id:text:"..bot_id..msg.chat_id_) then 
local rami_text = {
...
rami3 = math.random(#rami_text)
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"🎭"..rami_text[rami3].."\n🖼¦ صورك •⊱ "..all_photo_RAMBO.."\n🎟¦ ايديــك •⊱ ("..msg.sender_user_id_..")\n🎫¦ مـعرفك •⊱ "..rami_info.."\n🎖¦ رتبتـــك •⊱ "..t..'\n⭐️¦ تفاعـلك •⊱ '..formsgg(msguser).."\n💬¦ رسائلك •⊱ {"..user_msgs.."}\n🛠¦ تعديلاتك •⊱ {"..edit.."}\nֆ • • • • • • • ֆ ",msg.id_,msg.id_.."")
else 
local RAMBO_new_text = database:get("RAMBO:gr:id:text:"..bot_id..msg.chat_id_)
local RAMBO_new_text = RAMBO_new_text:gsub('#username',(rami_info or 'لا يوجد'))
...
database:set('RAMBO:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('RAMBO:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
database:set('RAMBO:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
redis:del('RAMBO:'..bot_id..'help', text)
redis:del('RAMBO:'..bot_id..'h1', text)
redis:del('RAMBO:'..bot_id..'h2', text)
redis:del('RAMBO:'..bot_id..'h3', text)
redis:del('RAMBO:'..bot_id..'h4', text)
redis:del('RAMBO:'..bot_id..'h5', text)
redis:del('RAMBO:'..bot_id..'h6', text)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('RAMBO:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'hhh'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('RAMBO:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if text:match("^الاوامر$") and is_mod(msg) then
local help = redis:get('RAMBO:'..bot_id..'help')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '💢┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('RAMBO:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'h11'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('RAMBO:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and is_mod(msg) then
local h1 = redis:get('RAMBO:'..bot_id..'h1')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('RAMBO:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'h22'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('RAMBO:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and is_mod(msg) then
local h2 = redis:get('RAMBO:'..bot_id..'h2')
local text =  [[
👮|℡ اوامر الادمنيه
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
⚙|℡ 《 كتم •⊱ الغاء 》كتم
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('RAMBO:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'h33'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('RAMBO:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and is_mod(msg) then
local h3 = redis:get('RAMBO:'..bot_id..'h3')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('RAMBO:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local rami2 = redis:get('RAMBO:'..bot_id..'h44'..msg.sender_user_id_..'')
if rami2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('RAMBO:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('RAMBO:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
...
local h4 = redis:get('RAMBO:'..bot_id..'h4')
local text =  [[
🔱|℡ اوامر المبرمجين
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
👨‍💻|℡ 《 تفعيل •⊱ تعطيل 》 ⇣
...
📮┊اهلا بك في سورس رامبو                      
  ﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
git clone https://github.com/ramizob3a/RAMBO && cd RAMBO && chmod +x install.sh && ./install.sh
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎  
✅┊طريقة التنصيب،☢️
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
...
♻️┊كود تشغيل البوت
  ./RAMBO/rm
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
📡|℡<a href="https://telegram.me/TH3VICTORY">قناه السورس○•</a>

📁|℡<a href="https://telegram.me/TH3VICTORY">ملفات السورس○•</a>

💬|℡<a href="t.me/TH3VICTORY_BOT">بوت تواصل السورس○•</a>
 
👨‍🔧|℡<a href="https://t.me/joinchat/AAAAAERvnA-zTcS4u5bOzQ">مطورين السورس👨‍🔧</a>
 
👨‍🔧|℡<a href="t.me/RAMBO_SYR">rami○•</a>

👨‍🔧|℡<a href="t.me/hdhul">y7ia○•</a>

👨‍🔧|℡<a href="t.me/SAHER_SYR">SAHER○•</a>

👨‍🔧|℡<a href="t.me/GENRALALI_SY">ALgeneral○•</a> 
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
...
if database:get("RAMBO:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local RAMBO_edit_text = database:get("RAMBO:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
...
text = msg.content_.text_
if is_sudo(msg) then 
if database:get("RAMBO:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
database:del("RAMBO:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('RAMBO:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'RAMBO:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('RAMBO:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'RAMBO:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('RAMBO:'..bot_id..'pro:groups', gpss[i]) then
...
id_rami = data.message_.content_.photo_.sizes_[0].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[1] then
id_rami = data.message_.content_.photo_.sizes_[1].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[2] then
id_rami = data.message_.content_.photo_.sizes_[2].photo_.persistent_id_
end	
if data.message_.content_.photo_.sizes_[3] then
id_rami = data.message_.content_.photo_.sizes_[3].photo_.persistent_id_
end
gpss = database:smembers( 'RAMBO:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('RAMBO:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendPhoto(gpss[i], 0, 0, 1, nil, id_rami,(msg.content_.caption_ or " "))
end					
end
end
gpss = database:smembers( 'RAMBO:'..bot_id.."groups") or 0
...
local rami1 = redis:get('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'')
if rami1 == 're' then
local rami2 = redis:get('RAMBO:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('RAMBO:'..bot_id..'rami'..rami2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('RAMBO:'..bot_id..':sticker:'..rami2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('RAMBO:'..bot_id..':voice:'..rami2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('RAMBO:'..bot_id..':video:'..rami2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('RAMBO:'..bot_id..':gif:'..rami2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('RAMBO:'..bot_id..':file:'..rami2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('RAMBO:'..bot_id..'ramire'..msg.chat_id_..'', rami2)
send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
end
local rami1 = redis:get('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'')
if rami1 == 're' then
local rami2 = redis:get('RAMBO:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('RAMBO:'..bot_id..'rami'..rami2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('RAMBO:'..bot_id..':sticker:'..rami2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('RAMBO:'..bot_id..':voice:'..rami2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('RAMBO:'..bot_id..':video:'..rami2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('RAMBO:'..bot_id..':gif:'..rami2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('RAMBO:'..bot_id..':file:'..rami2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('RAMBO:'..bot_id..'ramiresudo', rami2)
send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
redis:set('RAMBO:'..bot_id..'rami1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run RAMBO                         ««              --
...
local svuser = 'RAMBO:'..bot_id..'user:'..result.id_
if username then
...
if database:get('RAMBO:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('RAMBO:'..bot_id..'welcome:'..msg.chat_id_)
else
...
if database:get('RAMBO:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:RAMBO"..msg.chat_id_..bot_id) then
local pin_id = database:get('RAMBO:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local id_rami = msg.content_.document_.document_.persistent_id_
...
local url_rami3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_rami
local rami3 = https.request(url_rami3)
local rami6 = JSON.decode(rami3)
local photo_rami = download_to_file('https://api.telegram.org/file/bot'..token..'/'..rami6.result.file_path, 'files_RAMBO/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('RAMBO:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
if (not is_vip(msgg) and not text) then 
if database:get("RAMBO:lo:edit:new:"..bot_id..msgg.chat_id_) then 
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:RAMBO"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:RAMBO"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:RAMBO"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:RAMBO"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:RAMBO"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:RAMBO"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
